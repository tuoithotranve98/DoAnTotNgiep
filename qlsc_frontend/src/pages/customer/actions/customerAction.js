...
export const getListCustomer = (search = "", option = {}) => (
  dispatch,
  getState
) => {
  dispatch(updateCustomerFetching(true));
  dispatch(getFilterCustomers(search, option))
    .then((json) => {
...
export const getFilterCustomers = (search = "", option = {}) => (
  dispatch,
  getState
) => {
  const filter = processOption(search, option);
  return dispatch(
    fetch(`${API_CUSTOMER}/customers${filter}`, {
      method: "GET",
    })
  )
    .then((json) => {
